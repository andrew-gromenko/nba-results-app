...
export default ({ event: { competitions, status } }) => {
  const isOT = status.period === 5;
  return (
        ...
        <table>
          ...
          <tbody>
            {competitions[0].competitors.map(
              ({ id, team, winner, homeAway, linescores, score, records }) => (
                <tr className={winner ? styles.winner : styles.loser} key={id}>
                  <td className={styles["team-info"]}>
                    ...
                    <div>
                      ...
                      <div className={styles.records}>
                        ...
                        <span className={styles["home-away"]}>{homeAway}</span>)
                      </div>
                    </div>
                  </td>
